...
import com.cheapest.lansu.cheapestshopping.view.custom.pickerView.adapter.ArrayWheelAdapter;
import com.cheapest.lansu.cheapestshopping.view.custom.pickerView.lib.WheelView;
import com.cheapest.lansu.cheapestshopping.view.custom.pickerView.listener.OnItemSelectedListener;
...
public class WheelOptions<T> {
	...
	private Map<Integer, ArrayList<T>> mOptions2Items;
	private ArrayList<Map<Integer, ArrayList<T>>> mOptions3Items;
	...
	public void setPicker(ArrayList<T> options1Items,
						  Map<Integer, ArrayList<T>> options2Items, boolean linkage) {
...
//					int opt3 = wv_option3.getCurrentItem();//上一个opt3的选中位置
//					//新opt3的位置，判断如果旧位置没有超过数据范围，则沿用旧位置，否则选中最后一项
//					opt3 = opt3 >= mOptions3Items.get(opt2Select).size() - 1 ? mOptions3Items.get(opt2Select).size() - 1 : opt3;
//
//					wv_option3.setAdapter(new ArrayWheelAdapter(mOptions3Items.get(wv_option2.getCurrentItem()).get(opt3Select)));
//					wv_option3.setCurrentItem(opt3);
...
//		// 添加联动监听
	...
	private void itemSelected(int opt1Select, int opt2Select, int opt3Select) {
		...
		if (mOptions3Items != null) {
			wv_option3.setAdapter(new ArrayWheelAdapter(mOptions3Items.get(opt2Select).get(opt2Select)));
