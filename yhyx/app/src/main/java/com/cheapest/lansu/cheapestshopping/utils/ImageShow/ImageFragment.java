 ...
 * 图片fragment
 */
public class ImageFragment extends BaseFrag{
	...
	public static ImageFragment newInstance(String imgData, int width,
                                            int hight, int position, int x, int y,int type) {
	...
	protected void initOther() {
        ...
        type=getArguments() != null ? getArguments().getInt("type") : 2;
		bitMapHight = hight;
	...
	public void showPhoto(){
		...
		if (originalBitmapExist) {
			...
			if (position==0&&type!=1){
				new BitMaker(mContext, imgData, new BitmapMaker.loadImage() {
					@Override
	...
	public void loadThumbImg(final String url) {
		ImageLoader.getInstance().displayImage(url, photoView, new ImageLoadingListener() {
			...
			public void onLoadingFailed(String s, View view, FailReason failReason) {
				photoView.setImageResource(R.color.bg_gray);
				centerView();
			}
