...
public class BrowseActivity extends BaseActivity {
	...
	class HomeAdapter extends BaseQuickAdapter<CollctionEntity.DatasBean, BaseViewHolder> {
		...
		protected void convert(BaseViewHolder helper, CollctionEntity.DatasBean item) {
			helper.setText(R.id.item_home_pro_title, StringUtils.addImageLabel(BrowseActivity.this, item.getType() == 1 ? R.mipmap.icon_taobao : R.mipmap.icon_tianma, item.getName()));
				...
				public void onClick(View v) {
					for (int i = 0; i < mList.size(); i++) {
						if (item.getId() == mList.get(i).getId()) {
							cancelCollection(item.getCurrentId(), i);
							return;
						}
					}
	...
	public void getData() {
		RetrofitFactory.getInstence().API()
				.visitLis(CacheInfo.getUserID(this), page, 6)
	...
	public void cancelCollection(int id, final int position) {
		RetrofitFactory.getInstence().API()
				.deleteVisit("api/personal/visit/" + id + "/delete?userId=" + CacheInfo.getUserID(this))
				.subscribeOn(Schedulers.io())
				.observeOn(AndroidSchedulers.mainThread())
